...
export function ErrorState({message="Something went wrong",onRetry}:{message?:string;onRetry?:()=>void}){return <Card role="alert" className="p-8 text-center"><AlertCircle className="mx-auto mb-3 text-foreground" size={20}/><p className="text-sm font-medium">{message}</p>{onRetry&&<Button variant="outline" className="mt-4" onClick={onRetry}>Try again</Button>}</Card>}
